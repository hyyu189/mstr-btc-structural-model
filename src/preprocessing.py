...
def build_daily_panel(
    ...
    It then computes consistent structural quantities (assets, NAV, premium,
    BTC/share), tags distress / tiny-NAV dates, and returns both the full
    panel and a calibration subset.
    ...
    end_date:
        Optional ISO date string to cap the panel (e.g. ``\"2025-12-01\"``).
        If provided, the panel end date will be the minimum of this date and
        the available BTC, MSTR, and T-bill data.
    nav_calib_threshold:
        NAV threshold (in USD) for including observations in the calibration
        subset. Rows with ``nav_raw <= 0`` or ``nav_raw < nav_calib_threshold``
        are excluded from ``panel_calib``.
    """
    # Load raw series.
    ...
    nav_raw = asset_btc_usd - debt_total
    ...
    return PanelData(panel=df_out, panel_calib=calib)
