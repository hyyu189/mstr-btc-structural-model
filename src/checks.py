...
def run_smoke_checks() -> None:
    ...
    # Basic consistency checks on panel
    nav_raw = panel["nav_raw"].to_numpy()
    nav = panel["nav"].to_numpy()
    # Ignore leading rows before debt is defined (NaN).
    mask_nav = ~np.isnan(nav)
    assert np.all(nav[mask_nav] >= 0.0), "NAV must be non-negative."
    assert np.all(
        nav[mask_nav] >= np.maximum(nav_raw[mask_nav], 0.0) - 1e-6
    ), "NAV should be (A_t - D_t)^+."
    ...
    # Calibration
    params = build_model_params(panel, panel_calib)
