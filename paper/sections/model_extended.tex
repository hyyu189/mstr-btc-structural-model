...
We extend the single-debt baseline of Section~\ref{sec:methodology} to accommodate Strategy's
...
(cf.\ the debt process in Section~\ref{sec:methodology}).  This keeps the waterfall
...
\begin{equation}\label{eq:ca_nav}
  ...
  = \frac{A_t - D_s(t) - D_c(t) - P_{nc}(t)
          - \bigl(n_c(t) - n_c^{\mathrm{ITM}}(t)\bigr) F_c}
         {N_t^{\mathrm{sh}} + n_c^{\mathrm{ITM}}(t)\,\phi},
\end{equation}
where $n_c^{\mathrm{ITM}}(t)$ is the number of convertible preferred shares
for which $\phi\, P_t > F_c$ (i.e.\ in-the-money).  The numerator deducts
the par claim of out-of-the-money convertible preferred
$\bigl(n_c - n_c^{\mathrm{ITM}}\bigr) F_c$, since those shares retain
their liquidation preference rather than converting.
\end{definition}
...
\begin{equation}\label{eq:accretive}
  ...
  \frac{E_t}{A_t} > 1
  \quad\Longleftrightarrow\quad
  \pi_t > \log\!\bigl(\mathrm{ILE}_t\bigr).
\end{equation}
That is, ATM issuance is accretive when the log-premium exceeds
$\log(\mathrm{ILE}_t) = \log\!\bigl(A_t / (A_t - L_t)\bigr)$.
When $L_t = 0$ this reduces to $\pi_t > 0$; when $L_t > 0$ the
...
i.e.\ $E_t = P_t N_t^{\mathrm{sh}} > H_t S_t = A_t$.  Since
$E_t = e^{\pi_t}(A_t - L_t)$, this is equivalent to
$e^{\pi_t} > A_t / (A_t - L_t) = \mathrm{ILE}_t$, i.e.\
...
Table~\ref{tab:indicators_extended} summarizes the complete indicator suite.

\begin{table}[htbp]
\centering
\caption{Summary of structural indicators: base model and extensions.}
\label{tab:indicators_extended}
\small
